<?php
    ...
    $sql = 'SELECT
        ...
        update_dt,
        is_active
        FROM ticket_3 WHERE is_active = 0 OR ticket_id = "'.$ticket_id.'" ' ;
    ...
    while( $row=mysqli_fetch_array($query) ) {  
        ...
        $btnParam = array("id"=> $row['id'], "name"=> $row['name'], "telp"=> $row['telp'], "is_active"=> $row['is_active'], "ticket_type"=> 3);
        ...
        $nestedData[] = $count;
        $nestedData[] = "GCBB".sprintf("%03d", $row["id"]);
        $nestedData[] = $row["name"];
        $nestedData[] = $row["telp"];
