<?php
    ...
    $sql = 'SELECT
        ...
        update_dt,
        is_active
        FROM ticket_2 WHERE is_active = 0 OR ticket_id = "'.$ticket_id.'" ' ;
    ...
    while( $row=mysqli_fetch_array($query) ) {  
        ...
        $btnParam = array("id"=> $row['id'], "name"=> $row['name'], "telp"=> $row['telp'], "is_active"=> $row['is_active'], "ticket_type"=> 2);
        ...
        $nestedData[] = $count;
        $nestedData[] = "GCBA".sprintf("%03d", $row["id"]);
        $nestedData[] = $row["name"];
        $nestedData[] = $row["telp"];
